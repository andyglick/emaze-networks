...
public class IpRangeToCidrs implements Delegate<List<Cidr>, DenseRange<Ipv4>> {
    ...
    public List<Cidr> perform(DenseRange<Ipv4> range) {
        ...
        if (spanningCidr.last().equals(endIp)) {
            ...
            final List<Cidr> remainder = spanningCidr.exclude(previousIp);
        ...
        if (spanningCidr.first().equals(startIp)) {
            ...
            final List<Cidr> result = new ArrayList<>();
            final List<Cidr> remainder = spanningCidr.exclude(nextIp);
            for (Cidr cidr: remainder) {
                result.add(cidr);
        ...
        if (spanningCidr.first().compareTo(startIp) != Order.GT.order() && spanningCidr.last().compareTo(endIp) != Order.LT.order()) {
            ...
            final Ipv4 nextIp = endIp.next();
            final List<Cidr> remainder2 = result.removeLast().exclude(nextIp);
