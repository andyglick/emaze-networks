...
import net.emaze.networks.old.Ipv4ToDenseRange;
import net.emaze.networks.old.DenseRangeToIpv4;
...
public class DenseRangeToIpv4Test {
    ...
    public void canExtractBoundariesFromRange() {
        ...
        final DenseRange<Ipv4> range = new Ipv4ToDenseRange().perform(firstIp, lastIp);
        Assert.assertEquals(expected, new DenseRangeToIpv4().perform(range));
    }
    ...
    public void canExtractBoundariesFromHostRange() {
        ...
        final DenseRange<Ipv4> range = new Ipv4ToDenseRange().perform(address, address);
        Assert.assertEquals(expected, new DenseRangeToIpv4().perform(range));
    }
